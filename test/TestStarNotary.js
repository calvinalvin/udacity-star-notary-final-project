...
it('can add the star name and star symbol properly', async () => {
  ...
  assert.equal(await instance.name.call(), "Calvin's Star Notary");
...
it('lets 2 users exchange stars', async () => {
  ...
  let tokenId1 = 111;
  let tokenId2 = 222;
  await instance.createStar('A big star', tokenId1, {from: user1});
  await instance.createStar('A little star', tokenId2, {from: user2});
...
it('lets a user transfer a star', async () => {
  ...
  assert.equal(await instance.ownerOf(tokenId), user1);
...
it('lookUptokenIdToStarInfo test', async () => {
  ...
  // 1. create a Star with different tokenId
  await instance.createStar('Blah star', tokenId, {from: accounts[0]});

  // 2. Call your method lookUptokenIdToStarInfo
  assert(await instance.lookUptokenIdToStarInfo(tokenId), 'Blah star');
